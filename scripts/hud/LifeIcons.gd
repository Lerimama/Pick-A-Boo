...
func _process(delta: float) -> void:
	...
	self.life_count = Glo.game_manager.player_stats["player_life"]
...
func _on_value_change(new_value): # ne rabim parametra
	...
	# setam current life
	life_count = new_value # v bistvu Glo.game_manager.player_stats["player_life"]
	
	if previous_life > life_count:
		
		modulate = Glo.color_red
		yield(get_tree().create_timer(0.5), "timeout")
		modulate = Color.white
	
	elif previous_life < life_count:
		
		modulate = Glo.color_green
		yield(get_tree().create_timer(0.5), "timeout")
		modulate = Color.white
